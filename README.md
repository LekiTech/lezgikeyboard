...
The palochka (vertical bar) is inserted as Cyrillic Palochka **ӏ** (U+04CF); the bundled `lezgi_words.sqlite` dictionary stores Latin **I** (U+0049), so the prefix is normalized before querying.
...
| `WordSuggestions.swift` | SQLite reader for word suggestions. |
| `EmojiData.swift` | Generated emoji catalog by category — regenerate with `scripts/gen_emoji.py`, don't edit by hand. |
...
- [x] **Suggestion bar hit areas** — the full suggestion cell area is tappable, with a key-colored rounded press highlight
- [x] **Spacebar cursor control** — long-press and drag on spacebar moves the insertion point horizontally and between lines. Caveat: unlike the native keyboard, extensions cannot move the caret geometrically — vertical movement works via `\n`-separated lines from the (often truncated) document context, so it is approximate and cannot follow visual wraps of long lines
- [x] **Emoji page redesign** — the native emoji keyboard is not accessible from custom extensions, so the page was rebuilt to match it: full Unicode catalog (1,898 emoji in 8 categories, generated by `scripts/gen_emoji.py`), recent emojis, horizontal column scrolling, category bar with precise jumps, press highlight, hold-to-repeat delete
