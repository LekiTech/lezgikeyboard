...
/// Bounding box of pixels with alpha above a small threshold, in image coordinates.
...
/// Crop away the transparent margins so the artwork fills its cell in Messages,
/// keeping the canvas square (centered on the artwork) with a 3% breathing edge.
func trimmed(_ image: CGImage) -> CGImage {
    let bounds = opaqueBounds(of: image)
    let side = min(CGFloat(image.width), max(bounds.width, bounds.height) * 1.06)
    var square = CGRect(x: bounds.midX - side / 2, y: bounds.midY - side / 2,
                        width: side, height: side)
    square.origin.x = max(0, min(square.origin.x, CGFloat(image.width) - side))
    square.origin.y = max(0, min(square.origin.y, CGFloat(image.height) - side))
...
func renderImageSticker(from source: URL, to url: URL) {
    ...
    let original = loadImage(source)
    let image = trimmed(original)
    if image.width < original.width {
        let pct = 100 - image.width * 100 / original.width
        print("  (\(source.lastPathComponent) trimmed \(pct)% of empty margin)")
    }
